...
namespace AnimatedModelProject
{
    ...
    public class IsisBehavior : Behavior
    {
        ...
        protected override void Update(TimeSpan gameTime)
        {
            ...
            // Key up for advance!
            if (this.GoUp || inputService.KeyboardState.Up == WaveEngine.Common.Input.ButtonState.Pressed)
            {
                isMoving = true;
                rotation = Vector3.Zero;

                // Key left shift for RUN!
                if (this.Run || inputService.KeyboardState.LeftShift == WaveEngine.Common.Input.ButtonState.Pressed)
                {
                    // Run you fools!
            ...
            if (this.GoDown || inputService.KeyboardState.Down == WaveEngine.Common.Input.ButtonState.Pressed)
            {
                isMoving = true;
            ...
            // Key up for advance!
            if (this.GoRight || inputService.KeyboardState.Right == WaveEngine.Common.Input.ButtonState.Pressed)
            {
                isMoving = true;
            ...
            if (this.GoLeft || inputService.KeyboardState.Left == WaveEngine.Common.Input.ButtonState.Pressed)
            {
                isMoving = true;
                rotation = new Vector3(0, MathHelper.PiOver2, 0);
            ...
            this.transform3D.Position += (advance * (float)gameTime.TotalSeconds/100);
